...
const VersionSideBar = ({
	...
	return (
		<div
			className={`max-h-full h-fit transition-transform duration-200 ease-in-out rounded-tr-md rounded-br-md flex flex-col w-54 bg-[#181818] border border-white/2`}
		>
						...
						<>
							{messageToDisplay?.versionNo && (
								<div className="flex justify-center items-center font-medium">
									...
									<p className="text-base text-white/80 tracking-widest">
						...
						{messages &&
							messages.map((m, index) => (
								...
								>
									...
									<div
										className={`flex gap-5 items-center z-100 w-full pt-3 px-1 pb-9 ${messageToDisplay && messageToDisplay.id === m.id ? "bg-[#ffffff07] border-r-2 border-[#488AED]/50 " : " bg-transparent "}  hover:bg-[#222222] rounded-sm cursor-pointer relative`}
									>
										{/* <div>
											{m.status &&
												m.status === "COMPLETED" && (
